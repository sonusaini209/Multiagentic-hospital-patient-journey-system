...
# ── DB_DIR: uses /tmp on Vercel (read-only fs), local folder otherwise ──
...
You ONLY have access to your own database.
...
- ALWAYS JOIN all relevant tables
- Return ALL useful columns
- Use LEFT JOIN for result/item tables
...
Admissions(AdmissionID, PatientID, DoctorID, AdmitDate, DischargeDate, Ward, Status, Reason)
"""

_LABTRACK_SCHEMA = """
LabTests   (TestID, TestName, Description, NormalRange, UnitCost)
TestOrders (OrderID, PatientID, AdmissionID, TestID, OrderedDate, Status)
TestResults(ResultID, OrderID, ResultValue, ReportDate, Flag, Notes)
"""
...
_BILLDESK_SCHEMA = """
Bills    (BillID, PatientID, AdmissionID, TotalAmount, PaidAmount, Status, GeneratedDate)
BillItems(ItemID, BillID, Description, Amount, Category)
Insurance(InsuranceID, PatientID, Provider, PolicyNumber, CoverageAmount, ExpiryDate, Status)
"""
